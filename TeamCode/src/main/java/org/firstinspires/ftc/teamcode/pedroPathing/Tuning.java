...
class CentripetalTuner extends OpMode {
    ...
    public void start() {
        ...
        forwards = new Path(new BezierCurve(
            new Pose(), new Pose(Math.abs(DISTANCE), 0),
            new Pose(Math.abs(DISTANCE), DISTANCE)
        ));
        backwards = new Path(
            new BezierCurve(
                new Pose(Math.abs(DISTANCE), DISTANCE), new Pose(Math.abs(DISTANCE), 0),
                new Pose(0, 0)
            ));
    ...
    public void start() {
        circle = follower.pathBuilder()
            ...
            .addPath(new BezierCurve(
                new Pose(RADIUS, RADIUS), new Pose(RADIUS, 2 * RADIUS),
                new Pose(0, 2 * RADIUS)
            ))
            .setHeadingInterpolation(HeadingInterpolator.facingPoint(0, RADIUS))
            .addPath(new BezierCurve(
                new Pose(0, 2 * RADIUS), new Pose(-RADIUS, 2 * RADIUS),
...
class Drawing {
    ...
    public static void drawDebug(Follower follower) {
        if (follower.getCurrentPath() != null) {
            ...
            drawRobot(
                new Pose(
                    closestPoint.getX(), closestPoint.getY(), follower.getCurrentPath()
