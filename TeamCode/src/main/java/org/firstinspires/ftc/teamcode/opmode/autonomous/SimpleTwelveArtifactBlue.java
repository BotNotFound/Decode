...
@Autonomous(name = "12 Artifact Blue (simple)", group = "blue", preselectTeleOp = TwoPersonTeleOpBlue.OP_MODE_NAME)
public class SimpleTwelveArtifactBlue extends AutonomousBase {
    public SimpleTwelveArtifactBlue() {
        super(new Pose(106.8, 135.414, Math.toRadians(0)).mirror(), Robot.AllianceColor.BLUE);
    }
    ...
    protected AutonomousStage[] buildStageSequence() {
        ...
        return new AutonomousStage[]{
                ...
                new AutonomousStage(
                        ...
                        ).setConstantHeadingInterpolation(Math.toRadians(180-0)).build(),
                        Robot.RobotState.NONE
                ),
        };
    }

    @SuppressWarnings("PointlessArithmeticExpression") // see rationale at top of file
    public static class Paths {
        ...
        public Paths(Follower follower) {
            shootPreloads = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(106.800, 135.414).mirror(), new Pose(95.344, 99.816).mirror())
                    )
            ...
            goToFirstRowArtifacts = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(95.344, 99.816).mirror(), new Pose(90, 84).mirror())
                    )
            ...
            intakeFirstRow = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(90, 84).mirror(), new Pose(140, 84).mirror())
                    )
            ...
            shootFirstRow = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(140, 84).mirror(), new Pose(99.101, 100.353).mirror())
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180-0), Math.toRadians(180-45))
                    .build();
            ...
            goToThirdRow = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(91.051, 92.482).mirror(), new Pose(90, 37).mirror())
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180-45), Math.toRadians(180-0))
                    .build();
            ...
            shootThirdRow = follower
                    .pathBuilder()
                    .addPath(
                            new BezierLine(new Pose(150, 37).mirror(), new Pose(91.051, 92.482).mirror())
                    )
                    .setLinearHeadingInterpolation(Math.toRadians(180-0), Math.toRadians(180-45))
                    .build();
        }
